...
public class Ch06Controller {
	...
	@PostMapping("/login")
	public String lofin(String mid, String mpassword, HttpSession session) {
		String loginResult = "";
		if(mid.equals("admin")) {
	...
	public void fileDownload(String fname, HttpServletResponse response, HttpServletRequest request) throws Exception {
		...
		//IE11 이하 버전에서 한글 파일을 복원하기 위해
		//String downloadName = URLEncoder.encode("이백이.jpg", "UTF-8");
		//Webkit 기반 브라우저 (Chrome,Safari, FireFox, Opera, Edge)에서 한글 파일을 복원하기 위해
		//String downloadName = new String("이백이.jpg".getBytes("UTF-8"), "ISO-8859-1");		
	...
	public String jsonDownload1(Model model) {
		...
		board.setBno(100);
		board.setBtitle("공부하고싶다.");
		board.setBcontent("까짓것 하면 되지...빡공~!");
		board.setWriter("박도현");
